...
import type { DeviceCodeTemplateItem } from '../types/api';
...
export function DeviceCodeTemplatePicker({ onSelect, children }: Props) {
  const [open, setOpen] = useState(false);
  const [selectedTemplateId, setSelectedTemplateId] = useState<string | null>(null);
  const [selectedPlatform, setSelectedPlatform] = useState<string | null>(null);

  const { data: templates, isLoading } = useQuery({
    queryKey: ['device-code-templates'],
    queryFn: () => api.get<DeviceCodeTemplateItem[]>('/api/admin/modules/device-code-templates'),
    enabled: open,
  });
  ...
  const handleSelectTemplate = (template: DeviceCodeTemplateItem) => {
          ...
          <div className="flex h-[80vh] w-full max-w-4xl overflow-hidden rounded-lg bg-white shadow-lg">
            <div className="flex w-full flex-col">
              <div className="flex items-center justify-between border-b border-gray-200 p-4">
                <h3 className="text-lg font-semibold text-gray-900">Device Code Templates</h3>
